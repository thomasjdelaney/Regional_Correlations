...
def plotRegionalHistogram(correlation_frame, region, bin_width, stim_id, x_col, x_label, x_lim, y_lim):
    ...
    # plt.title('width = ' + str(bin_width) + 's')
    plt.legend()
    plt.tight_layout()

def plotRegionalCorrelationHistogram(correlation_frame, region, stim_id, bin_width, prefix):
    plotRegionalHistogram(correlation_frame, region, bin_width, stim_id, 'corr_coef', r'$r_{SC}$', [-1,1], [0,175])
...
def plotRegionalInfoHistogram(correlation_frame, region, stim_id, bin_width, prefix, max_mi):
    plotRegionalHistogram(correlation_frame, region, bin_width, stim_id, 'mutual_info', r'$I(X;Y)$ (bits)', [0, max_mi], [0,310])
