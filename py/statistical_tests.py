...
def getRegionToHistDict(correlation_frame, bin_width, measure, max_mi):
    region_to_hist = {}
    if measure == 'corr_coef':
        hist_bins = np.linspace(-1,1,25)
    elif measure == 'mutual_info':
        hist_bins = np.linspace(0,max_mi,25)
    else:
        sys.exit(dt.datetime.now().isoformat() + ' ERROR: ' + 'Unrecognised measure!')
    for region in rc.regions:
        ...
        region_to_hist[region] = np.histogram(region_bin_frame[measure], bins=hist_bins)[0]
    return region_to_hist

def getKSStatPVal(regions, region_to_corr_hist, region_to_info_hist):
    corr_stat, corr_p_val = mannwhitneyu(region_to_corr_hist[regions[0]], region_to_corr_hist[regions[1]])
    info_stat, info_p_val = mannwhitneyu(region_to_info_hist[regions[0]], region_to_info_hist[regions[1]])
...
def main():
    ...
    region_to_corr_hist = getRegionToHistDict(correlation_frame, args.bin_width, 'corr_coef', max_mi)
    region_to_info_hist = getRegionToHistDict(correlation_frame, args.bin_width, 'mutual_info', max_mi)
    region_combinations = combinations(rc.regions, 2)
    stats_p_values = np.array([getKSStatPVal(regions, region_to_corr_hist, region_to_info_hist) for regions in region_combinations])
